    ...
    public class ProcessActionsForFlow
    {
        ...
        public async Task ProcessActionsForFlowAsync(Guid flowId, List<BaseAction> allActions)
        {
            ...
            try
            {
                ...
                var updatedFlow = await _dbContext.Flows.FirstOrDefaultAsync(f => f.FlowId == flowId);

                if (flow != null)
                {
                    // Wenn nur ein Lauf erlaubt ist → StillPending auf false setzen
                    if (!flow.multipleRuns)
                    {
                        flow.stillPending = false;
                    }
